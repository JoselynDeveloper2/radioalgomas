            ...
            @if ($article->user->bio)
                <div class="bg-gray-50 dark:bg-gray-800 rounded-lg p-6 mb-12">
                    <div class="flex items-start">
                        <div
                            class="w-16 h-16 bg-gray-300 dark:bg-gray-600 rounded-full flex items-center justify-center mr-4 flex-shrink-0">
                            <span class="text-lg font-semibold text-gray-700 dark:text-gray-300">
                                {{ substr($article->user->name, 0, 1) }}
                            </span>
                        </div>
                        <div class="flex-1">
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">{{ $article->user->name }}
                            </h3>
                            <p class="text-gray-600 dark:text-gray-300 mb-3">{{ $article->user->bio }}</p>
                            <div class="flex space-x-4">
                                @if ($article->user->website)
                                    <a href="{{ $article->user->website }}" target="_blank"
                                        class="text-blue-600 dark:text-blue-400 hover:underline">
                                        Sitio web
                                    </a>
                                @endif
                                @if ($article->user->twitter)
                                    <a href="https://twitter.com/{{ $article->user->twitter }}" target="_blank"
                                        class="text-blue-600 dark:text-blue-400 hover:underline">
                                        Twitter
                                    </a>
                                @endif
                                @if ($article->user->linkedin)
                                    <a href="{{ $article->user->linkedin }}" target="_blank"
                                        class="text-blue-600 dark:text-blue-400 hover:underline">
                                        LinkedIn
                                    </a>
                                @endif
                                ...
                                <div class="p-4">
                                    <div class="flex items-center mb-2">
                                        <span class="inline-block px-2 py-1 text-xs font-semibold rounded-full"
                                            style="background-color: {{ $relatedArticle->category->color }}20; color: {{ $relatedArticle->category->color }}">
                                            {{ $relatedArticle->category->name }}
                                        </span>
                                    </div>
                                    <h3
                                        class="text-sm font-bold text-gray-900 dark:text-white mb-2 hover:text-blue-600 dark:hover:text-blue-400 transition-colors line-clamp-2">
